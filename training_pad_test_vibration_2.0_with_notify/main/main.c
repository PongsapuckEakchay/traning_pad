...
static uint8_t g_vibration_count=0;
static uint8_t g_vibration_reading = 0;
...
void vibration_timer(void* pvParameters){
    ...
    while(1){
        ...
        if (ulNotificationValue > 0) {
            ...
            // if(g_vibration_reading==0)
            // ESP_LOGI(Vibration_tag, "vibration_reading = %u", g_vibration_reading);
            // if(g_vibration_reading==1){
            //     gpio_set_intr_type(VIBRATION_PIN, GPIO_INTR_DISABLE);
            //     ESP_LOGI(Vibration_tag, "Stop timer 2");
            //     xTaskNotifyGive(Vibration_Handle);
            // }
        }
    }
...
void play_mml(const char* mml)
{
    ...
    while (*p) {
        ...
        if (note == 't' && isdigit((unsigned char)*p)) {  // Change tempo
            tempo = 0;
